...
function scoreTypography(stats) {
  ...
  return { points: Math.min(2, points), found, per300: +per300(found.reduce((a, f) => a + f.count, 0), wordCount).toFixed(2) };
}
...
export function analyze(text, preparedInfo = {}) {
  ...
  const emPoints = Math.min(5, emPer300 * 2);
  ...
  // typography matches are CMS smart-quote artifacts, not voice tells - keep out of the match list
  const matchList = deduped
    .filter((m) => m.category !== "typography")
    .slice(0, MAX_MATCHES)
    .map((m) => ({
      category: m.category,
      term: m.term,
